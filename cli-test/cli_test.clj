...
(deftest parse-test
  ...
  (testing "support directories"
    (is (= {"test-resources/test.yml" #ordered/map([:apiVersion "v1"]
            ...
            ;; in reality, `parse` interprets hcl1 as hcl2 unless explicitly stated that it's hcl1
            "test-resources/test.hcl1.tf" {"output" {"client_certificate" [{"value" "${google_container_cluster.primary.master_auth.0.client_certificate}"}],
                                           ...
                                           "provider" {"google" [{"project" "instrumenta",
                                                                  "region" "europe-west2",
                                                                  "version" "2.5.0"}]},
                                           "resource" {"google_container_cluster" {"primary" [{"name" "my-gke-cluster",
                                                                                               "remove_default_node_pool" true,
                                                                                               "initial_node_count" 1.0,
                                                                                               "location" "us-central1",
                                                                                               "master_auth" [{"username" "",
                                                                                                               "password" ""}]}]},
                                                       "google_container_node_pool" {"primary_preemptible_nodes" [{"cluster" "${google_container_cluster.primary.name}",
                                                                                                                   "location" "us-central1",
                                                                                                                   "name" "my-node-pool",
